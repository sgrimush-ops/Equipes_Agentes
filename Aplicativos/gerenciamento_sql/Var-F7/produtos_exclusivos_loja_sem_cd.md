...
SELECT * FROM (
    WITH ENTRADAS_LOJA AS (
        ...
        WHERE A.DTAENTRADA >= TRUNC(SYSDATE) - 180
          ...
          AND A.STATUSNF != 'C'
          AND A.NROEMPRESA IN (1, 2, 3, 4, 5, 6, 7, 8, 11, 12, 13, 14, 16, 17, 18)
    ),
    ENTRADAS_CD AS (
        SELECT /*+ MATERIALIZE */ DISTINCT B.SEQPRODUTO
        FROM MLF_NOTAFISCAL A
        ...
        WHERE A.DTAENTRADA >= TRUNC(SYSDATE) - 180
          AND A.CODGERALOPER = 1
          AND A.STATUSNF != 'C'
          AND A.NROEMPRESA NOT IN (1, 2, 3, 4, 5, 6, 7, 8, 11, 12, 13, 14, 16, 17, 18)
    )
    SELECT DISTINCT
    ...
    LEFT JOIN (
        ...
        WHERE NROEMPRESA NOT IN (1, 2, 3, 4, 5, 6, 7, 8, 11, 12, 13, 14, 16, 17, 18)
        GROUP BY SEQPRODUTO
