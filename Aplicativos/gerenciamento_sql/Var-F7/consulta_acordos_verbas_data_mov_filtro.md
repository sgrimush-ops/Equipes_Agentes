...
## Objetivo
Listar acordos comerciais e verbas (`MSUV_ACORDOPROMOC` / `MSU_ACORDOPROMOC`) que tiveram **movimentação ou atualização de quitação no período de :DT1 a :DT2**, com filtros opcionais por **Usuários da Manutenção (`:LT3`)** e **Tipos de Título / Espécies (`:LT4`)**, unificando todos os títulos financeiros vinculados (`FI_TITULO` / `FI_TITOPERACAO`), valores rateados rigorosamente por produto sem dízimas, totais gerais e auditoria de alteração com usuário e data/hora da última movimentação financeira.

## Arquitetura e Otimização
- **`CTE_TITULOS_ACORDO`**: Unifica via 6 uniões seguras todos os títulos vinculados ao acordo no Contas a Receber (`MSU_ACORDOTITULORECEB`, `MSU_CCACORDOPROMOC`, `MSU_CCDIVIDA`, `FI_TITOPERACAO` via processo e relacionamento direto por documento/espécie) aplicando o filtro multi-valores de **Tipos de Título (`:LT4`)** diretamente nas origens.
...
- **`CTE_TITULOS_GLOBAL`**: Cruza títulos e operações financeiras mapeando o parcelamento (`DENSE_RANK`), quitações e a flag de movimentação no período (`TEVE_MOV_QUITACAO_PERIODO = 1` quando quitação ou operação estiver entre `:DT1` e `:DT2`).
- **`CTE_ACORDO_RESUMO_GLOBAL`**: Agrega os dados financeiros consolidados por acordo (`NROACORDO`, `NROEMPRESA`).
- **`CTE_VALORES_BRUTOS`**: Aplica o filtro de quitação no período (`TG.TEVE_MOV_QUITACAO_PERIODO = 1`), filtros de loja (`#LT1`), expurgo de cancelados (`#LT2`), **usuários da manutenção (`:LT3`)** via busca nativa `INSTR`, comprador (`:LS1`), fornecedor (`:LS2`) e número do acordo (`:NR1`).
...
### LT4
- **Tipo:** Literal
- **Descrição:** Tipos de Titulo (Especies)
- **Valor padrão:** `0` (ou deixe vazio / `TODOS` para trazer todas as espécies, ou digite espécies separadas por vírgula sem espaço, ex: `boniac,devrec`)
- **Instrução p/ o usuário:** Digite uma ou mais espécies/tipos de título separados por vírgula sem espaço (ex: `boniac,devrec`). O filtro não diferencia maiúsculas/minúsculas. Para considerar todos os tipos de títulos, informe `0` ou `TODOS`.
...
3. Cadastrar as variáveis:
   ...
   - `LT4` (Literal) - Tipos de Titulo (Especies)
