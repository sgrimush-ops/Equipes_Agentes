# Var-F7 - abc_venda_formato_comprador_liquido_ponto_extra

## Query vinculada
- Arquivo SQL: `Aplicativos/gerenciamento_sql/querys/abc_venda_formato_comprador_liquido_ponto_extra.sql`

## Objetivo
Consulta de Curva ABC por Fornecedor, Subcategoria/Subgrupo e Produto com custos e margens líquidas, **incluindo a soma dos mínimos e máximos dos Pontos Extras ativos (cuja vigência não expirou)** com os mínimos e máximos já existentes no sistema (`MRL_PRODUTOEMPRESA`). Caso o produto/loja não possua ponto extra ativo ou o ponto extra tenha expirado, a consulta traz apenas os valores normais de estoque mínimo e máximo do sistema.

Permite filtragem pré-execução por Loja (`NR1`), Produto (`NR2`), Fornecedor (`LT2`), Loja de Custo CD (`LT1`), Comprador (`LS3`), Período (`DT1` e `DT2`) e Subcategoria (`LS1`).

## Regra de Negócio do Ponto Extra na Query
- Os saldos adicionais de ponto extra são consolidados na CTE `PONTO_EXTRA` via tabelas `MRL_PONTOEXTRA` e `MRL_PONTOEXTRAPRODUTOEMPRESA`.
- Considera apenas registros com status ativo (`PE.STATUS = 'A'` e `PEPE.STATUS = 'A'`).
- Valida que a data do ponto extra não expirou (`TRUNC(PEPE.DTAVIGENCIAFIM) >= TRUNC(SYSDATE)` e `NVL(TRUNC(PEPE.DTAVIGENCIAINICIO), TRUNC(SYSDATE)) <= TRUNC(SYSDATE)`).
- Os campos calculados no grid final são:
  - `EST_MIN`: `NVL(Y.ESTQMINIMOLOJA, 0) + NVL(PX.ESTQMINIMO_PE, 0)`
  - `EST_MAX`: `NVL(Y.ESTQMAXIMOLOJA, 0) + NVL(PX.ESTQMAXIMO_PE, 0)`
...
2. Colar ou carregar o SQL de `abc_venda_formato_comprador_liquido_ponto_extra.sql`.
