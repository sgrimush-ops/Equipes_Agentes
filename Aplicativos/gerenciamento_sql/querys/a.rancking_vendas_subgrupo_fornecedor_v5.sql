SELECT TO_CHAR(B.CODIGO_FORNECEDOR) AS CODIGO,
       NVL(B.FORNECEDOR_PRINCIPAL,'SEM DESCRICAO') AS FORNECEDOR,
      DECODE(NVL(UPPER(TRIM(:frmQVConsultaExecuta.LT2)),'D'),'C','CONSOLIDADO',MIN(B.COMPRADOR)) AS COMPRADOR,
  ...
  FROM (
        SELECT FP.CODIGO_FORNECEDOR,
               ...
               NVL(E.VLR_DEVOLUCAO_COMPRA,0) AS VLR_DEVOLUCAO_COMPRA,
          ...
          JOIN (
                SELECT V2.SEQPRODUTO,
                 ...
                 WHERE V1.TIPNOTAFISCAL = 'S'
                   ...
                   AND NVL(V1.MODELO,'0') <> '65'
                   AND V1.DTAEMISSAO >= TRUNC(:DT1)
                   AND V1.DTAEMISSAO < TRUNC(:DT2) + 1
                   AND V1.NROEMPRESA IN (1,2,3,4,5,6,7,8,11,12,13,14,15,16,17,18)
          ...
          LEFT JOIN (
                    SELECT X.SEQPRODUTO,
                           ...
                           SUM(CASE WHEN X.CGO_EFETIVA IN (802,860) THEN X.VLRITEM ELSE 0 END) VLR_DEVOLUCAO_COMPRA,
         ...
         WHERE (NVL(TRIM(#LS1),'0 - TODOS') IN ('0 - TODOS','TODOS','') OR (INSTR(#LS1,' - ')>0 AND FP.CODIGO_FORNECEDOR = TO_NUMBER(SUBSTR(#LS1,1,INSTR(#LS1,' - ')-1))))
           AND (NVL(TRIM('#NR1'),'0') = '0' OR FP.CODIGO_FORNECEDOR IN (#NR1))
           AND NOT EXISTS (
                 SELECT 1
 ...
 WHERE NVL(UPPER(TRIM(:frmQVConsultaExecuta.LT2)),'D') IN ('D','C')
 GROUP BY B.CODIGO_FORNECEDOR,
          B.FORNECEDOR_PRINCIPAL,
          CASE WHEN NVL(UPPER(TRIM(:frmQVConsultaExecuta.LT2)),'D') = 'C' THEN '1' ELSE B.COMPRADOR END
