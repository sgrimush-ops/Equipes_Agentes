SELECT
  CASE
    WHEN :LT2 = '0' AND NVL(UPPER(TRIM(:LT4)), 'TODOS') <> 'TODOS' THEN TO_CHAR(MAX(NVL(FP.CODIGO_FORNECEDOR, 0))) || ' - ' || MAX(NVL(FP.FORNECEDOR_PRINCIPAL, 'SEM DESCRICAO'))
    WHEN :LT2 = '0' THEN '0 - TODOS'
    ELSE :LT2 || ' - ' || MAX(NVL(FP.FORNECEDOR_PRINCIPAL, 'SEM DESCRICAO'))
  END AS TITULO_FORNECEDOR,
  SUB.SUBGRUPO AS SUBGRUPO,
  A.SEQPRODUTO AS COD_PROD,
  SUBSTR(A.DESCCOMPLETA, 1, 55) AS PRODUTO,
  MAX(FD.PADRAOEMBCOMPRA) AS EMB,
  TO_CHAR(MAX(NVL(C3.PRECO_CUSTO, 0)), 'FM99990D00', 'NLS_NUMERIC_CHARACTERS='',.''') AS PRC_CST,
