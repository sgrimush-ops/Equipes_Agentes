SELECT * FROM (
    ...
    CTE_TITULOS_GLOBAL AS (
        SELECT /*+ MATERIALIZE */
            ...
            CASE 
                WHEN (TA.DTAQUITACAO IS NOT NULL AND TRUNC(TA.DTAQUITACAO) BETWEEN ADD_MONTHS(TRUNC(SYSDATE), -10) AND ADD_MONTHS(TRUNC(SYSDATE), 3))
                  OR (OP.MAX_DTAOPERACAO IS NOT NULL AND TRUNC(OP.MAX_DTAOPERACAO) BETWEEN ADD_MONTHS(TRUNC(SYSDATE), -10) AND ADD_MONTHS(TRUNC(SYSDATE), 3))
                  OR (OP.MAX_DTAHORAALTERACAO IS NOT NULL AND TRUNC(OP.MAX_DTAHORAALTERACAO) BETWEEN ADD_MONTHS(TRUNC(SYSDATE), -10) AND ADD_MONTHS(TRUNC(SYSDATE), 3))
                THEN 1 ELSE 0 
    ...
    CTE_VALORES_BRUTOS AS (
        ...
        WHERE A.NROEMPRESA IN (1, 2, 3, 4, 5, 6, 7, 8, 11, 12, 13, 14, 15, 16, 17, 18, 50, 900)
          AND A.STATUS NOT IN (2)
          AND UPPER(TRIM(TG.USU_ULTIMA_ALTERACAO)) IN ('VERIDI', 'DEBCO', 'JORENE')
