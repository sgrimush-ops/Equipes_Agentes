SELECT TO_CHAR(B.CODIGO_FORNECEDOR) AS COD_F,
       ...
       'R$ ' || TO_CHAR(ROUND(SUM(B.VLR_COMPRA) - SUM(B.VLR_DEVOLUCAO_COMPRA) - SUM(B.VLR_TROCA_COMPRA),2),'999G999G999G990D00') AS VLR_COMPRA,
  ...
  FROM (
        SELECT FP.CODIGO_FORNECEDOR,
               ...
               NVL(DEV.VLR_DEVOLUCAO_COMPRA,0) + NVL(E.VLR_DEVOLUCAO_ENTRADA,0) AS VLR_DEVOLUCAO_COMPRA,
               NVL(DEV.VLR_TROCA_COMPRA,0) + NVL(E.VLR_TROCA_ENTRADA,0) AS VLR_TROCA_COMPRA,
          ...
          LEFT JOIN (
                      SELECT X.SEQPRODUTO,
                             ...
                             SUM(CASE WHEN X.CGO_EFETIVA IN (802) THEN X.VLRITEM ELSE 0 END) VLR_DEVOLUCAO_ENTRADA,
                             SUM(CASE WHEN X.CGO_EFETIVA IN (860) THEN X.VLRITEM ELSE 0 END) VLR_TROCA_ENTRADA
                        FROM (
                              SELECT N.SEQPRODUTO,
                                     NVL(N.VLRITEM,0) VLRITEM,
                                     CASE WHEN N.TIPPEDCOMPRAITEM = 'B' AND NF.TIPPEDIDOCOMPRA = 'C' AND REGEXP_LIKE(MP.VALOR, '^[0-9]+$') THEN TO_NUMBER(MP.VALOR)
          ...
          LEFT JOIN (
                SELECT I.SEQPRODUTO,
                 ...
                 WHERE N.CODGERALOPER IN (802, 860)
                   ...
                   AND N.STATUSNF != 'C'
                   AND N.NROEMPRESA IN (1, 2, 3, 4, 5, 6, 7, 8, 11, 12, 13, 14, 15, 16, 17, 18, 21, 50)
          ...
          LEFT JOIN (
                    SELECT I.SEQPRODUTO,
                           SUM(NVL(I.VLRITEM,0)) QTD_INCINERACAO_ANO
                      FROM MLFV_BASENFE N
                     ...
                     WHERE N.CODGERALOPER = 831
                       ...
                       AND N.DTAEMISSAO >= TO_DATE('01/02/2026', 'DD/MM/YYYY')
         ...
         WHERE ((:LS1 = '0 - TODOS') OR NVL(FD.SEQCOMPRADOR, 0) = TO_NUMBER(SUBSTR(:LS1, 1, INSTR(:LS1, ' - ') - 1)))
           ...
           AND (VF.SEQPRODUTO IS NOT NULL OR E.SEQPRODUTO IS NOT NULL OR INCI.SEQPRODUTO IS NOT NULL OR DEV.SEQPRODUTO IS NOT NULL)
           AND NOT EXISTS (
                  SELECT 1
 ...
 HAVING SUM(B.VLR_VENDA) > 0 OR SUM(B.VLR_COMPRA) > 0 OR SUM(B.VLR_BONIFICADO) > 0 OR SUM(B.VLR_DEVOLUCAO_COMPRA) > 0 OR SUM(B.VLR_TROCA_COMPRA) > 0
 ORDER BY SUM(B.VLR_COMPRA - B.VLR_DEVOLUCAO_COMPRA - B.VLR_TROCA_COMPRA) DESC, 2 ASC
