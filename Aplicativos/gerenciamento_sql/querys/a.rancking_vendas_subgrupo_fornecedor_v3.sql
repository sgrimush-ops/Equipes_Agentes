SELECT TO_CHAR(B.CODIGO_FORNECEDOR) AS CODIGO_FORNECEDOR,
      ...
      DECODE(NVL(UPPER(TRIM(:LT2)),'D'),'C','CONSOLIDADO',MIN(B.COMPRADOR)) AS COMPRADOR,
       ...
       'R$ ' || TO_CHAR(ROUND(SUM(B.QTD_INCINERACAO_ANO_ATUAL),2),'999G999G999G990D00') AS QTD_INCINERACAO_ANO_ATUAL
  FROM (
        SELECT FP.CODIGO_FORNECEDOR,
               FP.FORNECEDOR_PRINCIPAL,
               NVL(COMP.COMPRADOR,'SEM GESTOR') AS COMPRADOR,
               A.SEQPRODUTO,
               NVL(INCI.QTD_INCINERACAO_ANO_ATUAL,0) AS QTD_INCINERACAO_ANO_ATUAL,
          ...
          LEFT JOIN (
                    SELECT I.SEQPRODUTO,
                           SUM(NVL(I.QUANTIDADE,0)) QTD_INCINERACAO_ANO_ATUAL
                      FROM MLFV_BASENFE N
