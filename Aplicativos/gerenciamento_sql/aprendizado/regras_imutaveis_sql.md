...
- **Injeção Obrigatória de MATERIALIZE**: Em todas as CTEs que leem Views do Consinco (`MACV_`, `MAXV_`, `MRLV_`, etc.) ou tabelas de alto volume, usar obrigatoriamente a dica `/*+ MATERIALIZE */` (ex: `SELECT /*+ MATERIALIZE */ ...`) para obrigar o banco Oracle a criar a tabela em memória RAM antes dos joins principais, evitando reexecuções múltiplas da mesma view.
- **Bypass do Parser e Proibição de Fallback sem CTE**: A ferramenta de validação do Totvs SGI/Consinco exige que a primeira palavra do script seja `SELECT`. É ESTRITAMENTE PROIBIDO criar versões alternativas sem CTE (ex: scripts `_fallback`) para contornar o erro *"não é uma consulta"*. A remoção das CTEs destrói a performance no Oracle. A solução única e obrigatória é "envelopar" toda a consulta CTE com um `SELECT * FROM ( WITH ... SELECT ... )`. Para detalhes arquiteturais e comparativos, ler obrigatoriamente `aprendizado_otimizacao_ctes_materialize.md`.
