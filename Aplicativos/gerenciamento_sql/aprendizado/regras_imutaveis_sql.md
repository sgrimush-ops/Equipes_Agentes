...
- **Bypass do Parser (Erro "não é uma consulta")**: A ferramenta de validação do Totvs SGI/Consinco exige que a primeira palavra do script seja `SELECT`. Ao criar estruturas otimizadas usando blocos CTE (`WITH ...`), o parser rejeita o código antes de enviar ao Oracle. A solução obrigatória é "envelopar" toda a query com um `SELECT * FROM ( ... sua query completa ... )`. Lembre-se de não esquecer o fechamento de parênteses e colocar qualquer cláusula `ORDER BY` na parte de fora do envelope.
