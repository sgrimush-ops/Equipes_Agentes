...
class CalibrationWindow(tk.Toplevel):
    def __init__(self, parent):
        ...
        self.coords = {"cd_abastecedor": None, "nova_linha": None}
    ...
    def start_capture(self, key):
        btn = self.btn_busca if key == "cd_abastecedor" else self.btn_qtd
    ...
    def update_ui_after_capture(self, key, x, y, btn, orig_text):
        btn.config(text=orig_text if "Aponte" not in orig_text else ( "1. Capturar 'CD Abastecedor'" if key == "cd_abastecedor" else "2. Capturar 'Nova Linha'" ), state="normal")
        lbl = self.lbl_busca if key == "cd_abastecedor" else self.lbl_qtd
