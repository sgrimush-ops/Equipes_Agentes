...
class OrderProcessor:
    ...
    def run(self, update_callback=None, stop_event=None, pause_event=None, store_id=None):
        """
        Executa a automação para CD 016 lendo digitar.csv.
        """
        if not self.coords:
            if update_callback:
        ...
        try:
            ...
            items_processed = 0
            unique_stores = valid_rows['Código Empresa'].unique()

            for current_store in unique_stores:
                if stop_event and stop_event.is_set(): return
                
                store_rows = valid_rows[valid_rows['Código Empresa'] == current_store]
                ...
                # na sequencia precisamos de 5 tab, para chegar na empresa de faturamento; digitar nesse campo 016
...
def main():
    ...
    print("=== Robô de Digitação CD 016 (Modo Automático) ===")
