...
class InversaCalibrationWindow(tk.Toplevel):
    def __init__(self, parent):
        super().__init__(parent)
        self.title("Calibração - Ajuste de Seleção Inversa")
        self.geometry("450x350")
        
        self.coords = {}
        self.load_existing_coords()
        
        # Configurar as chaves esperadas para esta ação
        self.expected_keys = [
            "familia_inversa",
            "divisao_inversa",
            "categoria_inversa",
            "abastecimento_inversa"
        ]
            
        self.buttons = {}
        self.labels = {}
        
        tk.Label(self, text="Mapeamento de Botões (Seleção Inversa)", font=("Segoe UI", 12, "bold")).pack(pady=10)
        tk.Label(self, text="Mapeie os 4 locais exatos onde o robô deve clicar.", wraplength=400).pack(pady=5)
        
        container = tk.Frame(self)
        container.pack(fill="both", expand=True, padx=10, pady=5)
        
        self.create_calibration_row(container, "1. Coordenada: Família", "familia_inversa")
        self.create_calibration_row(container, "2. Coordenada: Divisão", "divisao_inversa")
        self.create_calibration_row(container, "3. Coordenada: Categoria (Duplo Clique)", "categoria_inversa")
        self.create_calibration_row(container, "4. Coordenada: Forma Abastecimento", "abastecimento_inversa")
    ...
    def save_coords(self):
        ...
        # Atualiza apenas as chaves da nossa tela
