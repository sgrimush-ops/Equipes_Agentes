...
from core.digitador_pedido_unico import OrderProcessorUnico

class AcaoDigitarPedidoUnico(BaseAction):
    @property
    def name(self) -> str:
        return "Digitar Pedido Único"
    ...
    def execute(self, update_callback=None, stop_event=None, pause_event=None):
        if update_callback:
            update_callback({'status': 'Preparando digitação único...'})
            
        processor = OrderProcessorUnico()
        # O método run da classe OrderProcessorUnico já lida com stop_event e pause_event
        processor.run(
