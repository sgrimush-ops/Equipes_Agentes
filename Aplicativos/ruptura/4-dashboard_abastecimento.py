...
def compute_metrics(df_subset, forma_nome):
    """Computa as métricas de ruptura para um subconjunto de dados (Forma ou Geral)."""
    if df_subset.empty:
        return pd.DataFrame()
...
def principal():
    ...
    # 1. Visão Global (TODAS AS FORMAS)
    df_todas = compute_metrics(df, "TODOS")
    master_frames.append(df_todas)
    
    # 2. Visão por Forma
    formas = sorted([x for x in df['FORMA_ABASTECIMENTO'].dropna().unique()])
    for c in formas:
        print(f"Computando Forma {c}...")
        df_c = compute_metrics(df[df['FORMA_ABASTECIMENTO'] == c], str(c))
    ...
    for forma_filter in final_df['FORMA_ABASTECIMENTO_FILTER'].unique():
        mask_comp = final_df['FORMA_ABASTECIMENTO_FILTER'] == forma_filter
    ...
    for c in formas:
        options_formas += f'<option value="{c}">{c}</option>'
    ...
    <body>
                    ...
                    <div class="row mt-3">
                        <div class="col-md-6" style="font-size: 0.95rem;">
                            <ul class="list-unstyled">
                                <li class="mb-2"><strong>Base Loja:</strong> Número total de produtos únicos que deveriam estar ativos na gôndola desta filial (no cenário geral ou no mix do forma selecionado).</li>
        ...
        function atualizarDashboard() {
            ...
            let dadosForma = masterData.filter(d => d.FORMA_ABASTECIMENTO_FILTER === forma);
            
            let dadosTabela = [];
            let dadosGrafico = [];

            if (loja === "TODAS") {
                dadosTabela = [...dadosForma];
                dadosGrafico = dadosForma.filter(d => d.LOJA !== "TOTAL GERAL");
            } else {
                dadosTabela = dadosForma.filter(d => d.LOJA === loja || d.LOJA === "TOTAL GERAL");
                dadosGrafico = dadosForma.filter(d => d.LOJA === loja);
            }

            renderTable(dadosTabela);
            renderChart(dadosGrafico, "Ranking: " + (forma === "TODOS" ? "Todas as Formas de Abastecimento" : forma) + " | Foco: " + (loja === "TODAS" ? "Rede Completa" : loja));
        }
