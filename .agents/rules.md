...
- **Consultas SQL (Comentários):** NUNCA adicione comentários textuais (como `--` ou `/* */`) na estrutura de scripts SQL. Em editores de consulta do Totvs Consinco, qualquer comentário iniciado por `--` causará falhas de execução no interpretador do ERP.
- **Consultas SQL (Erros ORA-00904 e Nomenclatura):** Na tabela `MAP_FAMDIVISAO`, a coluna de ligação com a divisão comercial chama-se `NRODIVISAO` (ex: `FD.NRODIVISAO = 1`), e não `NROEMPRESA`. Assumir `NROEMPRESA` causará o erro `ORA-00904: "FD"."NROEMPRESA": invalid identifier`.
...
- **Consultas SQL (Incineração / Métricas):** Em relatórios consolidados que envolvam a coluna de Incineração, certifique-se de somar o valor financeiro (`I.VLRITEM`) e não a quantidade física (`I.QUANTIDADE`), a menos que o usuário solicite explicitamente a quantidade de SKUs. A discrepância entre unidades e valores em reais é uma armadilha comum que gera diferenças na comparação com relatórios fiscais.
