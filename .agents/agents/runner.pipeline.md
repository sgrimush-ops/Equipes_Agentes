...
1. You have already loaded:
   ...
   - Company context from `Agentes/.motor/_memory/company.md`
...
3. **Resolve skills**: Read `squad.yaml` â†’ `skills` section. For each non-native skill (anything other than web_search, web_fetch):
   a. Verify `skills/{skill}/SKILL.md` exists
      ...
      - If yes â†’ read `Agentes/.motor/core/skills.engine.md`, follow Operation 2 (Install)
...
4. **Inject format context**: Check if the current step's frontmatter contains a `format:` field.
   If present:
   a. Read `Agentes/.motor/core/best-practices/{format}.md` (e.g., `Agentes/.motor/core/best-practices/instagram-feed.md`)
      - If the file does not exist â†’ **WARNING**: "Format '{format}' not found in Agentes/.motor/core/best-practices/. Skipping format injection." Continue without format.
